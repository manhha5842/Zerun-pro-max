...
export const ACCOUNT_KIND_OPTIONS: Array<{ value: AccountKind; label: string; description: string }> = [
  { value: "source", label: "Nguồn", description: "Tài khoản dùng để crawl hoặc lấy nội dung đầu vào." },
  { value: "target", label: "Đích", description: "Tài khoản dùng để đăng bài hoặc xuất bản nội dung." }
];

export const PLATFORM_OPTIONS: Array<{ value: AccountPlatform; label: string; description: string; icon: ReactNode }> = [
  { value: "facebook", label: "Facebook", description: "Phù hợp cho page, profile hoặc group đăng bài.", icon: <Facebook aria-hidden size={16} /> },
  ...
  { value: "x", label: "X / Twitter", description: "Dùng username/password/email/2FA secret khi cần.", icon: <Twitter aria-hidden size={16} /> },
  ...
  { value: "instagram", label: "Instagram", description: "Tài khoản Instagram cơ bản bằng username/password.", icon: <Instagram aria-hidden size={16} /> }
];
...
export function validateDraft(draft: AccountDraft): FormErrors {
  ...
  if (draft.platform === "threads") {
    if (!draft.threadsSessionDir.trim() && (!draft.threadsUsername.trim() || !draft.threadsPassword.trim())) {
      errors.threadsSessionDir = "Nhập sessionDir hoặc cung cấp Instagram username/password.";
...
function PlatformSpecificFields({ draft, errors, setDraft }: PlatformFieldsProps) {
  if (draft.platform === "facebook") {
    return <FacebookAccountForm draft={draft} errors={errors} setDraft={setDraft} />;
  }
  if (draft.platform === "telegram") {
    return <TelegramAccountForm draft={draft} errors={errors} setDraft={setDraft} />;
  }
  if (draft.platform === "x") {
    return <XAccountForm draft={draft} errors={errors} setDraft={setDraft} />;
  }
  if (draft.platform === "threads") {
    return <ThreadsAccountForm draft={draft} errors={errors} setDraft={setDraft} />;
  }
...
export function AccountForm({
  ...
  const helpTitle = useMemo(() => {
    return fixedKind === "target" ? "Mẫu phù hợp để thêm tài khoản đăng bài" : "Mẫu phù hợp để thêm tài khoản crawl";
  }, [fixedKind]);
    ...
    <form className="panel panel-pad account-form-shell" onSubmit={handleSubmit}>
      <div className="account-form-header">
        ...
        <div className="kind-pill">
          ...
          <span>{fixedKind === "source" ? "Nguồn" : "Đích"}</span>
        </div>
      </div>

      <InlineNote>
        <strong>{helpTitle}.</strong> Nếu cần flow nhiều bước hơn, hãy dùng trang <em>Tài khoản</em> để tạo account tập trung.
      </InlineNote>
      ...
      <div className="form-grid" style={{ marginTop: 14 }}>
        <div className="field">
          ...
          <Input id={`${fixedKind}-name`} value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Facebook Page A" />
          <FormError message={errors.name} />
        </div>
        <div className="field">
          <Label htmlFor={`${fixedKind}-platform`}>Nền tảng</Label>
          <Select
            id={`${fixedKind}-platform`}
            value={draft.platform}
            onChange={(event) => setDraft((current) => ({ ...current, platform: event.target.value as AccountPlatform }))}
          >
        ...
        <div className="field full">
          <Label htmlFor={`${fixedKind}-handle`}>Handle / URL mặc định</Label>
          <Input
            id={`${fixedKind}-handle`}
            value={draft.handle}
            onChange={(event) => setDraft((current) => ({ ...current, handle: event.target.value }))}
            placeholder="@page, @channel hoặc URL profile/group"
          />
      ...
      <div className="platform-card-grid">
        {PLATFORM_OPTIONS.map((option) => (
          <button
            key={option.value}
            type="button"
            className={`platform-choice ${draft.platform === option.value ? "active" : ""}`}
            onClick={() => setDraft((current) => ({ ...current, platform: option.value }))}
          >
      ...
      <div className="form-grid" style={{ marginTop: 12 }}>
        <div className="field full">
          ...
          <Textarea
            id={`${fixedKind}-credentials`}
            value={draft.credentialsText}
            onChange={(event) => setDraft((current) => ({ ...current, credentialsText: event.target.value }))}
        ...
        <div className="field full">
          ...
          <Textarea
            ...
            placeholder={'{\n  "campaignId": "access_trade_campaign"\n}'}
          />
      ...
      <div className="actions" style={{ marginTop: 16 }}>
        <Button disabled={isSubmitting}>{isSubmitting ? "Đang lưu..." : submitLabel ?? "Thêm cấu hình"}</Button>
        <Button type="button" variant="ghost" onClick={() => { setDraft(createEmptyDraft(fixedKind, draft.platform)); setErrors({}); }} disabled={isSubmitting}>
          Làm trống form
        </Button>
        <div className="muted-chip">
          <Sparkles aria-hidden size={14} />
          <span>Validation theo nền tảng + JSON + session path</span>
        </div>
      </div>
    </form>
  );
