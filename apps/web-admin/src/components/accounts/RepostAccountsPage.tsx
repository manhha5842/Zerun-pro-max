...
import { platformLabel, type AccountKind, type RepostAccount } from "../../pages/repostTypes";
...
import { AccountChannelsManager } from "./AccountChannelsManager";
...
export function RepostAccountsPage({ kind }: { kind: AccountKind }) {
  const queryClient = useQueryClient();
  const toast = useToast();
  const isSource = kind === "source";
  const collectionPath = isSource ? "/sources" : "/targets";
  const collectionKey = isSource ? "sources" : "targets";
  const endpointKey = isSource ? "source" : "target";
  ...
  useEffect(() => {
    if (session?.status === "login_ok" && stage === "connect") {
      setStage("ready");
      toast.success("Đăng nhập Zalo thành công. Bạn có thể thêm nhiều nhóm ở phần quản lý kênh.");
    }
  ...
  const completeTelegram = async (login: TelegramLoginResponse["login"]) => {
    ...
    setStage("ready");
    toast.success("Đăng nhập Telegram thành công. Bạn có thể thêm nhiều nhóm hoặc kênh bên dưới.");
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title={isSource ? "Tài khoản nguồn" : "Tài khoản đích"}
        subtitle={isSource
          ? "Tài khoản chỉ giữ phiên đăng nhập. Sau khi kết nối, bạn chọn nhiều kênh nguồn ở khu quản lý bên dưới."
          : "Kết nối tài khoản đăng riêng nếu cần. Bạn cũng có thể dùng lại phiên của tài khoản nguồn khi thêm kênh đích."}
      ...
      {showSetup ? (
        <SectionCard
          ...
          description="Nhóm và kênh không cấu hình ở đây; một tài khoản có thể được dùng cho nhiều kênh."
        ...
        >
          ...
          {stage === "details" ? (
            <div className="account-wizard-panel">
              ...
              <div className="form-grid">
                <label className="span-2"><Label>Tên tài khoản</Label><Input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Ví dụ: Telegram A" /></label>
                {form.platform === "telegram" ? <>
          ...
          {stage === "ready" ? (
            <div className="account-wizard-panel">
              <div className="setup-complete-card"><CheckCircle2 size={28} aria-hidden /><div><span className="eyebrow">Đã kết nối</span><h3>{form.name}</h3><p>Phiên đăng nhập đã sẵn sàng. Đóng wizard và thêm các kênh cần đọc hoặc đăng ở phần quản lý kênh.</p></div></div>
              <div className="actions">
                ...
                <Button variant="ghost" onClick={() => setShowSetup(false)}>Đến quản lý kênh</Button>
              </div>
            </div>
          ) : null}
        </SectionCard>
      ...
      <SectionCard title={isSource ? "Danh sách tài khoản nguồn" : "Danh sách tài khoản đích"} description="Tài khoản là phiên đăng nhập; danh sách kênh được quản lý độc lập bên dưới.">
        <FilterToolbar><Input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="Tìm tên, nền tảng, health..." /></FilterToolbar>
        <AdminDataTable
          rows={rows}
          getRowKey={(row) => row.id}
          empty={<EmptyState title="Chưa có tài khoản" description="Bấm Kết nối tài khoản để bắt đầu." />}
          columns={[
      ...
      <AccountChannelsManager role={kind} />
    </div>
  );
}
